...
import { useMealQueryWithDeleted } from '@/api/meals';
...
export function OrdersPage() {
    const PAGE_SIZE = 10;
    const { data: mealsData, isLoading: mealsLoading, isError: mealsError } = useMealQueryWithDeleted();
    ...
    if (mealsError) {
        return <div className="p-4">Failed to load meals.</div>;
    }

    const getMeal = (mealId: number): Meal | undefined => meals.find((meal) => meal.id === mealId);

    const getOrderTotal = (order: Order): number | null => {
        let total = 0;
        for (const item of order.items) {
            const meal = getMeal(item.meal_id);
            if (!meal) {
                return null;
            }
            total += item.quantity * meal.price;
        }
        return total;
    };
    ...
    return (
        <div className="px-4 py-8">
            ...
            <div className="space-y-6">
                ...
                >
                    {groupedOrders.map((ordersForDate) => (
                        <div key={getDateString(ordersForDate[0].created_at)} className="rounded-2xl border border-border bg-white/70 p-4 shadow-sm">
                            <div className="mb-3 flex items-center justify-between">
                                <h2 className="text-lg font-semibold">{getDateString(ordersForDate[0].created_at)}</h2>
                                <div className="text-xs text-muted-foreground">{ordersForDate.length} orders</div>
                            </div>
                                        ...
                                        <AccordionItem key={order.id} value={order.id.toString()}>
                                            <AccordionTrigger>
                                                <div className="py-2 flex flex-wrap gap-3 text-sm">
                                                    <div className="rounded-full bg-black/5 px-3 py-1">Order #{order.id}</div>
                                                    <div className="rounded-full bg-black/5 px-3 py-1">
                                                        {orderTotal === null ? 'Total unavailable' : `Total ${orderTotal.toFixed(2)} EUR`}
                                                    </div>
                                                    <div className="rounded-full bg-black/5 px-3 py-1">
                                            ...
                                            <AccordionContent>
                                                    ...
                                                    {order.items.map((item) => {
                                                        ...
                                                        return (
                                                            <div key={item.meal_id} className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2">
                                                                <div className="text-sm">{item.quantity}x {meal?.name ?? `Deleted meal #${item.meal_id}`}</div>
                                                                <div className="text-sm font-semibold">
                                                                    {meal ? `${(item.quantity * meal.price).toFixed(2)} EUR` : 'N/A'}
                                                                </div>
                                                            </div>
                                                        );
                                                    })}
                                                ...
                                                {order.review && (
                                                    <div>
                                                        <div className="mt-4 text-sm text-muted-foreground">{order.review.comment}</div>
